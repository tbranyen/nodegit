...
const windowsCommonConditions = [R.test(/^\s*os=Windows$/gm), R.test(/^\s*shared=False$/gm)];
...
const detectDistributionPairFromConfig = (itemHash, body) =>
  R.pipe(
...
const getDistributionConfig = (itemHash) =>
  got(getDistributionConfigURLFromHash(itemHash)).then(({ body }) =>
    detectDistributionPairFromConfig(itemHash, body)
  );
...
const discoverDistributions = (treeHtml) => {
  ...
  return Promise.all(R.map((releaseHash) => getDistributionConfig(releaseHash), releaseHashes));
};
