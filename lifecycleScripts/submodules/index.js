...
var gitExecutableLocation = require(path.join(rootDir, './utils/gitExecutableLocation'));
  ...
  return gitExecutableLocation()
    ...
    .then(function (statuses) {
      ...
      if (outOfSyncSubmodules.length) {
        console.warn('[nodegit] WARNING - Some submodules are pointing to an new commit:');
      ...
      return statuses
        ...
        .reduce(function (chainPromise, submoduleToUpdate) {
          return chainPromise.then(function () {
            ...
            return exec('git submodule update --init --recursive ' + submoduleToUpdate.name);
          });
        }, Promise.resolve());
    });
};
