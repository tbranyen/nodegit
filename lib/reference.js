  ...
  return NodeGit.Reference.lookup(repo, refName)
    .then((ref) => {
      if (ref.type() === NodeGit.Reference.TYPE.DIRECT) {
        ...
        return getTerminal(repo, ref.symbolicTarget(), depth - 1, ref).then(({ error, out }) => {
...
const getSignatureForReflog = (repo) => {
  ...
  return NodeGit.Signature.default(repo).catch(() => NodeGit.Signature.now('unknown', 'unknown'));
};

/**
 * Given a reference name, follows symbolic links and updates the direct reference to point to a
 * given OID. Updates the reflog with a given message.
 *
