...
function applySelectedLinesToTarget(originalContent, newLines, pathHunks, isStaged, reverse) {
  ...
  function processSelectedLine(hunkLine) {
    ...
    var newLine = newLines.filter(function (nLine) {
      return (
        hunkLine.oldLineno() === nLine.oldLineno() && hunkLine.newLineno() === nLine.newLineno()
      );
    });
 ...
 *
 * @param {Repository} repository      The repository that the rebase is being performed in.
 ...
 * @param {Function}   beforeNextFn    Callback to be called before each invocation of next(). If
 *                                     the callback returns a promise, the next() will be called
 *                                     when the promise resolves.
 * @param {Function}   beforeFinishFn  Callback called before the invocation of finish(). If the
 *                                     callback returns a promise, finish() will be called when the
 *                                     promise resolves. This callback will be provided a detailed
 *                                     overview of the rebase.
 * @returns {number | Index} An error code for an unsuccesful rebase or an index for a rebase with
 *                           conflicts.
 */
function performRebase(repository, rebase, signature, beforeNextFn, beforeFinishFn) {
  ...
  function readRebaseMetadataFile(fileName, continueOnError) {
    return fse
      .readFile(path.join(repository.path(), 'rebase-merge', fileName), { encoding: 'utf8' })
      .then(fp.trim)
      .catch(function (err) {
        if (continueOnError) {
          return null;
        ...
        if (error && error.errno === NodeGit.Error.CODE.ITEROVER) {
          ...
          const beforeFinishFnPromise = !beforeFinishFn
            ...
            : Promise.all([
              ...
              ]).then(function ([ontoName, ontoSha, originalHeadName, originalHeadSha, rewritten]) {
 ...
 * @param {number} acrossFs     If non-zero, then the lookup will not stop when a filesystem device
 *                              change is detected while exploring parent directories.
 * @param {string} ceilingDirs  A list of absolute symbolic link free paths.
 *                              the search will stop if any of these paths are hit. This may be set
 *                              to null.
 * @returns {string} Path of the git repository.
 * @async
 */
...
/**
 * This will set the HEAD to point to the local branch and then attempt to update the index and
 * working tree to match the content of the latest commit on that branch.
 *
...
/**
 * This will set the HEAD to point to the reference and then attempt to update the index and working
 * tree to match the content of the latest commit on that reference.
 *
...
Repository.prototype.checkoutRef = function (reference, opts) {
  ...
  return repo
    .getReferenceCommit(reference.name())
 ...
 *
 * @param {Signature}     signature       Identity of the one performing the rebase.
 * @param {Function}      beforeNextFn    Callback to be called before each step of the rebase. If
 *                                        the callback returns a promise, the rebase will resume
 *                                        when the promise resolves. The rebase object is is passed
 *                                        to the callback.
 * @param {Function}      beforeFinishFn  Callback called before the invocation of finish(). If the
 *                                        callback returns a promise, finish() will be called when
 *                                        the promise resolves. This callback will be provided a
 *                                        detailed overview of the rebase.
 * @param {RebaseOptions} rebaseOptions   Options to initialize the rebase object with.
 * @returns {Oid | Index} A commit id for a succesful merge or an index for a rebase with conflicts.
 * @async
 */
Repository.prototype.continueRebase = function (
  ...
  return promiseChain
    ...
    .then((_rebase) => {
      rebase = _rebase;
      return rebase.commit(null, signature).catch((e) => {
 ...
 * @param {Array}        parents
 * @returns {Oid} The oid of the commit.
 * @async
 */
 ...
 * @param {Array}        parents
 * @returns {string} The content of the commit object as a string.
 * @async
 */
Repository.prototype.createCommitBuffer = function (author, committer, message, treeOid, parents) {
  ...
  return Promise.all(promises).then(function ([tree, ...parentCommits]) {
      ...
      switch (code) {
        ...
        default: {
          const error = new Error(
            'Repository.prototype.createCommitWithSignature ' + `threw with error code ${code}`
          );
          error.errno = code;
          throw error;
        }
 ...
 * @param {Signature} committer
 * @param {string}    message
 * @returns {Oid} The oid of the new commit.
 * @async
 */
Repository.prototype.createCommitOnHead = function (filesToAdd, author, committer, message) {
  var repo = this;
 ...
 * @param {string | Oid} String   Sha or Oid.
 * @param {string}       name     The name of the tag.
 * @param {string}       message  The description that will be attached to the annotated tag.
 * @returns {Tag}
 * @async
 */
 ...
 * @param {Array}  selectedLines  The array of DiffLine objects selected for discarding.
...
Repository.prototype.discardLines = function (filePath, selectedLines, additionalDiffOptions) {
  ...
  return repo
    ...
    .then(function (newContent) {
      return FilterList.load(repo, null, filePath, Filter.MODE.SMUDGE, Filter.FLAG.DEFAULT).then(
 ...
 * @param {Object | FetchOptions} fetchOptions  Options for the fetch, includes callbacks for
 *                                              fetching.
 * @async
 */
Repository.prototype.fetch = function (remote, fetchOptions) {
  var repo = this;
...
/**
 * Fetches from all remotes. This is done in series due to deadlocking issues with fetching from
 * many remotes that can happen.
 *
 * @param {Object | FetchOptions} fetchOptions  Options for the fetch, includes callbacks for
 *                                              fetching.
 * @async
...
/**
 * @param {FetchheadForeachCb} callback  The callback function to be called on each entry.
 * @async
 */
 ...
 *
 * @param {string | Remote} remote
 * @returns {Promise<Remote>} The remote object.
 * @async
 */
...
/**
 * Get extended statuses of a repo to it's working directory. Status entries have `status`,
 * `headToIndex` delta, and `indexToWorkdir` deltas.
 *
 * @param {obj} opts
 * @returns {Promise<StatusFile>}
 * @async
...
Repository.prototype.rebaseBranches = function (
  ...
  return promiseChain
    ...
    .then(function (oid) {
      ...
      return NodeGit.Rebase.init(repo, branchCommit, upstreamCommit, ontoCommit, rebaseOptions)
 ...
 * @returns {Oid | Index} A commit id for a succesful merge or an index for a merge with conflicts.
 * @async
 */
Repository.prototype.mergeBranches = function (
  ...
  return promiseChain
    .then(() => Promise.all([repo.getBranch(to), repo.getBranch(from)]))
    .then((objects) => {
      toBranch = objects[0];
    ...
    .then((branchCommits) => {
      ...
      return NodeGit.Merge.base(repo, toCommitOid, fromCommitOid).then((baseCommit) => {
        if (baseCommit.toString() == fromCommitOid) {
          ...
          return NodeGit.Reference.lookup(repo, 'HEAD')
            ...
            .then(([oid, message]) => {
              ...
              return repo.createCommit(toBranch.name(), signature, signature, message, oid, [
                toCommitOid,
                fromCommitOid,
              ]);
            })
...
/**
 * @param {MergeheadForeachCb} callback  The callback function to be called on each entry.
 * @async
 */
 ...
 *
 * @param {string}  filePath       The relative path of this file in the repo.
 * @param {Array}   selectedLines  The array of DiffLine objects selected for staging or unstaging.
 * @param {boolean} isStaged       Are the selected lines currently staged.
 * @returns {number} 0 or an error code.
 * @async
 */
Repository.prototype.stageLines = function (
  filePath,
  selectedLines,
  isSelectionStaged,
