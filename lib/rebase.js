...
function defaultRebaseOptions(options, checkoutStrategy) {
  ...
  if (options) {
    ...
    if (options.signingCb) {
      const signingCb = options.signingCb;
 ...
 * @param {AnnotatedCommit} branch    The terminal commit to rebase, or NULL to rebase the current
 *                                    branch.
 * @param {AnnotatedCommit} upstream  The commit to begin rebasing from, or NULL to rebase all
 *                                    reachable commits.
 * @param {AnnotatedCommit} onto      The branch to rebase onto, or NULL to rebase onto the given
 *                                    upstream.
...
/**
 * Opens an existing rebase that was previously started by either an invocation of Rebase.open or by
 * another client.
 *
