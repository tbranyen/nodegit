...
var args = cov.concat(['test/runner', 'test/tests', '--expose-gc', '--timeout', '15000']);
...
function unencryptKey(fileName) {
  var base64Contents = fs.readFileSync(path.join(__dirname, fileName + '.enc'), 'utf8');
  var asciiContents = Buffer.from(base64Contents, 'base64').toString('ascii');
