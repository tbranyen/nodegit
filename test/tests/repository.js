...
describe('Repository', function () {
  ...
  it('can initialize a repository into a folder', function () {
    return Repository.init(newRepoPath, 1).then(function (_path, _isBare) {
      return Repository.open(newRepoPath);
    });
  });

  it('can utilize repository init options', function () {
    return fse.remove(newRepoPath).then(function () {
  ...
  it('can be cleaned', function () {
    ...
    return this.repository.getHeadCommit().then(function (commit) {
  ...
  it('can list remotes', function () {
    return this.repository.getRemoteNames().then(function (remotes) {
  ...
  it('can get a reference commit', function () {
    return this.repository.getReferenceCommit('master').then(function (commit) {
      assert.equal('32789a79e71fbc9e04d3eff7425e1771eb595150', commit.toString());
    });
  });

  it('can get the default signature', function () {
    this.repository.defaultSignature().then((sig) => {
  ...
  it('gets statuses with StatusFile', function () {
    ...
    return fse
      ...
      .catch(function (e) {
        return fse.remove(filePath).then(function () {
  ...
  function discover(ceiling) {
    ...
    return NodeGit.Repository.discover(testPath, 0, ceiling).then(function (foundPath) {
      assert.equal(expectedPath, foundPath);
    });
  }

  it('can discover if a path is part of a repository, null ceiling', function () {
    return discover(null);
  });

  it('can discover if a path is part of a repository, empty ceiling', function () {
    return discover('');
  });

  it('can create a repo using initExt', function () {
    var initFlags =
      NodeGit.Repository.INIT_FLAG.NO_REINIT |
  ...
  it('returns null if there is no head commit', function () {
    return this.emptyRepo.getHeadCommit().then(function (commit) {
      assert(!commit);
    });
  });
  ...
  it('can get all merge heads in a repo with mergeheadForeach', function () {
    ...
    return RepoUtils.createRepository(repoPath)
      ...
      .then(function () {
        return repo.mergeheadForeach(assertBranchTargetIs.bind(this, theirBranch));
      })
      .then(function () {
