...
describe('Index', function () {
  ...
  it('can get a conflict from the index', function () {
    ...
    return Repository.init(rebaseReposPath, 0)
      ...
      .then(function (conflict) {
        ...
        promises.push(
          repository.getBlob(conflict.ancestor_out.id).then(function (blob) {
            assert.equal(blob.toString(), baseFileContent);
          })
        );

        promises.push(
          repository.getBlob(conflict.our_out.id).then(function (blob) {
            assert.equal(blob.toString(), baseFileContent + ourFileContent);
          })
        );

        promises.push(
          repository.getBlob(conflict.their_out.id).then(function (blob) {
  ...
  it('can add a conflict to the index', function () {
    ...
    return RepoUtils.createRepository(repoPath)
      ...
      .then(function (index) {
        assert.ok(!index.hasConflicts());
        return index.conflictAdd(ancestorIndexEntry, ourIndexEntry, theirIndexEntry);
      })
      .then(function () {
        return repo.index();
      })
  ...
  it('can find the specified file in the index', function () {
    ...
    return test.index.find('src/wrapper.cc').then(function (position) {
  ...
  it('can find the specified prefix in the index', function () {
    ...
    return test.index.findPrefix('src/').then(function (position) {
      assert.notEqual(position, null);
    });
  });
  ...
  it('can find the prefix when a file shares the name', function () {
    ...
    return test.index.find('LICENSE').then(function (position) {
      assert.notEqual(position, null);
    });
  });
});
