...
describe('Commit', function () {
  ...
  it('can call getTree on a parent commit', function () {
    return this.commit
      ...
      .then(function (tree) {
        assert.equal(tree.id().toString(), '327ff68e59f94f0c25d2c62fb0938efa01e8a107');
      });
  });
    ...
    return NodeGit.Repository.open(reposPath)
      ...
      .then(
        function (commitId) {
          ...
          return undoCommit().then(function () {
            return reinitialize(test);
          });
        },
        function (reason) {
          return reinitialize(test).then(function () {
  ...
  it('can create a commit as a buffer', function () {
    ...
    return NodeGit.Repository.open(reposPath)
      ...
      .then(function (signatures) {
        ...
        return repo.createCommitBuffer(author, committer, 'message', treeOid, [parent]);
      })
      .then(
  ...
  it('can amend commit', function () {
    ...
    return NodeGit.Repository.open(reposPath)
      ...
      .then(function () {
        return fse.writeFile(path.join(repo.workdir(), newFileName), newFileContent);
      })
      .then(function () {
        return repo.refreshIndex();
      })
      ...
      .then(function (resultOid) {
        ...
        return Promise.all([
          ...
          NodeGit.Signature.create('New Foo Bar', 'newfoo@bar.com', 246802468, 12),
          NodeGit.Signature.create('New Foo A Bar', 'newfoo@bar.com', 4807891730, 32),
        ]);
      })
  ...
  describe('amendWithSignature', function () {
    it('can amend with signature', function () {
      ...
      return NodeGit.Repository.open(reposPath)
        ...
        .then(function (headCommit) {
          ...
          return headCommit.amendWithSignature(null, null, null, null, null, null, onSignature);
        })
        .then(function (oidResult) {
          oid = oidResult;
    ...
    it('will respects overridden arguments', function () {
      ...
      var author = NodeGit.Signature.create('Scooby Doo', 'scoob@mystery.com', 123456789, 60);
    ...
    it('can optionally skip signing process', function () {
      ...
      return NodeGit.Repository.open(reposPath)
        ...
        .then(function (commitResult) {
          commit = commitResult;
          return commit.getSignature('gpgsig').then(
            function () {
  ...
  it('can fetch a single parent', function () {
    return this.commit.parent(0).then(function (parent) {
      assert.strictEqual(parent.sha(), 'ecfd36c80a3e9081f200dfda2391acadb56dac27');
  ...
  describe("Commit's Body", function () {
    ...
    it('non-null when body exists', function () {
      ...
      return NodeGit.Commit.lookup(
        ...
        assert.equal(commit.body(), 'Added new methods in checkout and repository');
      });
    });
  });
  ...
  it('duplicates signature', function () {
    ...
    var startNonSelfFreeingCount = Signature.getNonSelfFreeingConstructedCount();
  ...
  describe("Commit's Signature", function () {
    ...
    it('Can create a signed commit in a repo and update existing ref', function () {
      var signedData =
        '-----BEGIN PGP SIGNATURE-----\n' +
        'Version: GnuPG v1.4.12 (Darwin)\n' +
        '\n' +
    ...
    it('Can create a signed commit in bare a repo and update non-existent ref', function () {
      var signedData =
        '-----BEGIN PGP SIGNATURE-----\n' +
        '\n' +
    ...
    it('Can create a signed commit raw', function () {
      ...
      return NodeGit.Repository.open(reposPath)
        .then(function (repoResult) {
          repo = repoResult;
          return Commit.createWithSignature(repo, commit_content, signature, 'gpgsig');
        })
        .then(function (commitId) {
          assert.equal(expectedCommitId, commitId);
          return NodeGit.Commit.lookup(repo, commitId);
    ...
    it('Can retrieve the gpg signature from a commit', function () {
      ...
      return NodeGit.Commit.lookup(this.repository, 'cc1401eaac4e9e77190e98a9353b305f0c6313d8')
        .then(function (commit) {
          return commit.getSignature();
        })
        .then(function (extractSignature) {
          assert.equal(extractSignature.signature, expectedSignature);
          assert.equal(extractSignature.signedData, expectedSignedData);
        });
    });
