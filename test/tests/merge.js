/* eslint-disable no-unused-vars -- there's too many */
...
describe('Merge', function () {
  ...
  beforeEach(function () {
    var test = this;
    return RepoUtils.createRepository(reposPath).then(function (repo) {
  ...
  it('can merge --no-ff a fast-forward using the convenience method', function () {
    var ourFileName = 'ourNewFile.txt';
    var theirFileName = 'theirNewFile.txt';
  ...
  it('can merge --no-ff a non-fast-forward using the convenience method', function () {
    ...
    return (
      fse
        .writeFile(path.join(repository.workdir(), initialFileName), initialFileContent)
        // Load up the repository index and make our initial commit to HEAD
        .then(function () {
          return repository.refreshIndex();
        })
  ...
  it('leaves repo in MERGE state after a standard merge with conflicts fails', function () {
    ...
    return fse
      ...
      .then(function () {
        assert.equal(repository.state(), NodeGit.Repository.STATE.MERGE);
      ...
      .then(function (oid) {
        return Promise.all([
          ...
          var msg = fse.readFileSync(path.join(repoGitPath, 'MERGE_MSG'), 'utf-8');
          assert.ok(msg);

          return repository.createCommit(ourBranch.name(), ourSignature, ourSignature, msg, oid, [
            commits[0],
            commits[1],
          ]);
        });
      ...
      .then(function (commitOid) {
        ...
        assert.equal(repository.state(), NodeGit.Repository.STATE.NONE);
        // verify the convenience method
  ...
  it('can retrieve error code on if common merge base not found', function () {
    ...
    return NodeGit.Repository.open(local('../repos/workdir'))
      ...
      .then(function (parents) {
        return NodeGit.Merge.base(repo, parents[0], parents[1]).then(
          function () {
