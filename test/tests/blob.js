...
describe('Blob', function () {
  ...
  function commitFile(repo, fileName, fileContent, commitMessage) {
    ...
    return fse
      .writeFile(path.join(repo.workdir(), fileName), fileContent)
      .then(function () {
  ...
  it('can get a blob with an Oid object', function () {
    ...
    return this.repository.getBlob(oidObject).then(function (blob) {
  ...
  describe('createFromBuffer', function () {
    ...
    it('throws an error when repository is null', function () {
      return Blob.createFromBuffer(null, null, 0).catch(function (error) {
    ...
    it('throws an error when buffer is null', function () {
      var test = this;
      return Blob.createFromBuffer(test.repository, null).catch(function (error) {
    ...
    it('throws an error when no length is provided', function () {
      var test = this;
      return Blob.createFromBuffer(test.repository, Buffer.from('testing')).catch(function (error) {
  ...
  describe('createFromDisk', function () {
    ...
    it('throws an error when the file cannot be found', function () {
      ...
      return Blob.createFromDisk(test.repository, 'aaaaaaaaaa').catch(function (error) {
        assert.equal(error.errno, -3);
      });
    });

    it('throws an error when repository is null', function () {
      return Blob.createFromDisk(null, null, 0).catch(function (error) {
        assert.strictEqual(error.message, 'Repository repo is required.');
      });
    });
    ...
    it('throws an error when path is null', function () {
      var test = this;
      return Blob.createFromDisk(test.repository, null).catch(function (error) {
  ...
  describe('createFromWorkdir', function () {
    ...
    it('throws an error when the file cannot be found', function () {
      ...
      return Blob.createFromWorkdir(test.repository, 'thisisabadfile.jpg').catch(function (error) {
        assert.equal(error.errno, -3);
      });
    });

    it('throws an error when repository is null', function () {
      return Blob.createFromWorkdir(null, null, 0).catch(function (error) {
        assert.strictEqual(error.message, 'Repository repo is required.');
      });
    });
    ...
    it('throws an error when path is null', function () {
      var test = this;
      return Blob.createFromWorkdir(test.repository, null).catch(function (error) {
        assert.strictEqual(error.message, 'String relative_path is required.');
      });
    });
  });
  ...
  describe('filteredContent (DEPRECATED)', function () {
    ...
    it('retrieves the filtered content', function () {
      ...
      return commitFile(test.repository, attrFileName, filter, 'added gitattributes')
        .then(function () {
          return commitFile(
            test.repository,
            newFileName,
        ...
        .then(function (lfBlob) {
          ...
          return Blob.filteredContent(test.lfBlob, newFileName, 0);
        })
        .then(function (content) {
    ...
    it('returns non-binary filtered content when checking binary', function () {
      ...
      return commitFile(test.repository, attrFileName, filter, 'added gitattributes')
        ...
        .then(function (lfBlob) {
          ...
          return Blob.filteredContent(test.lfBlob, newFileName, 1);
        })
        .then(function (content) {
          var ending = content.match(lineEndingRegex);
          assert.strictEqual(ending[0], '\r\n');
    ...
    it('returns nothing when checking binary blob', function () {
      ...
      return commitFile(test.repository, attrFileName, filter, 'added gitattributes')
        ...
        .then(function (binaryBlob) {
          ...
          return Blob.filteredContent(test.binaryBlob, newFileName, 1);
        })
        .then(function (content) {
    ...
    it('throws an error when the blob is null', function () {
      return Blob.filteredContent(null, '', 0).catch(function (err) {
        assert.strictEqual(err.message, 'Blob blob is required.');
      });
    });

    it('throws an error when the path is null', function () {
      var test = this;
      return Blob.filteredContent(test.blob, null, 0).catch(function (err) {
    ...
    it('throws an error when the flag is undefined', function () {
      var test = this;
      return Blob.filteredContent(test.blob, '').catch(function (err) {
        assert.strictEqual(err.message, 'Number check_for_binary_data is required.');
      });
    });
  });
  ...
  describe('filter', function () {
    ...
    it('returns non-binary filtered content when checking binary', function () {
      ...
      return commitFile(test.repository, attrFileName, filter, 'added gitattributes')
        ...
        .then(function (lfBlob) {
          ...
          return test.lfBlob.filter(newFileName, {
    ...
    it('returns nothing when checking binary blob', function () {
      ...
      return commitFile(test.repository, attrFileName, filter, 'added gitattributes')
        ...
        .then(function (binaryBlob) {
          ...
          return test.binaryBlob.filter(newFileName, {
            flags: NodeGit.Blob.FILTER_FLAG.CHECK_FOR_BINARY,
          });
        })
        .then(function (content) {
    ...
    it('returns blob when not checking binary on binary blob', function () {
      ...
      return commitFile(test.repository, attrFileName, filter, 'added gitattributes')
        ...
        .then(function (binaryBlob) {
          ...
          return test.binaryBlob.filter(newFileName, { flags: 0 });
        })
        .then(function (content) {
          assert.strictEqual(content, binary.toString());
        });
    ...
    it('throws an error when the path is null', function () {
      var test = this;
      return test.blob.filter(test.blob, null, { flags: 0 }).catch(function (err) {
        assert.strictEqual(err.message, 'String as_path is required.');
      });
    });
  });
