...
describe('Reference', function () {
  ...
  it('can compare two different references', function () {
    ...
    return this.repository.getReference('checkout-test').then(function (otherRef) {
      assert.notEqual(ref.cmp(otherRef), 0);
    });
  });

  it('will return undefined looking up the symbolic target if not symbolic', function () {
  ...
  it('can rename a reference', function () {
    ...
    return repo
      ...
      .then(function (reflog) {
        var refEntryMessage = reflog.entryByIndex(reflog.entrycount() - 1).message();
