...
describe('Stage', function () {
  ...
  function stagingTest(isUnstaged, newFileContent, discarding) {
    ...
    if (!isUnstaged || discarding) {
      ...
      workingDirFile = expectedContent
        .replace('Three', 'Changed three')
        .replace('Seventeen', 'Changed seventeen');
    }
    ...
    if (isUnstaged) {
      getDiffFunction = function () {
        return test.repository.refreshIndex().then(function (index) {
      ...
      getDiffFunction = function () {
        return RepoUtils.addFileToIndex(test.repository, fileName)
          ...
          .then(function (treeAndIndex) {
            ...
            return NodeGit.Diff.treeToIndex(test.repository, masterTree, index, {
              flags:
                NodeGit.Diff.OPTION.SHOW_UNTRACKED_CONTENT |
                NodeGit.Diff.OPTION.RECURSE_UNTRACKED_DIRS,
            });
    ...
    return RepoUtils.commitFileToRepo(test.repository, fileName, fileContent)
      .then(function () {
        return fse.writeFile(path.join(test.repository.workdir(), fileName), workingDirFile);
      })
      .then(function () {
      ...
      .then(function () {
        if (discarding) {
          return fse.readFile(path.join(test.repository.workdir(), fileName), 'utf8');
        }

        return test.repository.refreshIndex().then(function (reloadedIndex) {
  ...
  it('can stage second to last line with no newline at EOF', function () {
    var newlineEofTestFileContent2 =
      newlineEofTestFileContent +
      '\nSixteen lines of text\nSeventeen lines of text\nEighteen lines of text';
  ...
  it('staging last hunk stages whole file if no filemode changes', function () {
    return stagingTest(true, lastHunkStagedFileContent)
      ...
      .then(function (index) {
        return NodeGit.Diff.indexToWorkdir(test.repository, index, {
          flags:
            NodeGit.Diff.OPTION.SHOW_UNTRACKED_CONTENT | NodeGit.Diff.OPTION.RECURSE_UNTRACKED_DIRS,
        });
      })
      .then(function (diff) {
  ...
  if (process.platform == 'linux' || process.platform == 'darwin') {
    it('can stage filemode changes for one file', function () {
      ...
      function afterWriteFn(repo, fileName) {
        return fse.chmod(path.join(repo.workdir(), fileName), 0755 /* new filemode */);
      }

      return (
        createAndCommitFiles(test.repository, fileName, fileContent, afterWriteFn)
          //Then, diff between head commit and workdir should have filemode change
          .then(function () {
            return compareFilemodes(true, null, 0111 /* expect +x */).then(function () {
          ...
          .then(function (oid) {
            return test.repository.getHeadCommit().then(function (parent) {
      ...
      return (
        createAndCommitFiles(test.repository, fileName, fileContent, afterWriteFn)
          ...
          .then(function () {
            ...
            return compareFilemodes(false, index, 0111 /* expect +x */).then(function () {
    ...
    it('can stage/unstage filemode changes for one file', function () {
      ...
      function afterWriteFn(repo, fileName) {
        ...
        return exec('git update-index --chmod=+x ' + fileName, { cwd: repo.workdir() })
          .then(function () {
            //Commit the change with execute bit set
            return exec("git commit -m 'test'", { cwd: repo.workdir() });
          })
          .then(function () {
  ...
  it('can stage/unstage filemode changes for multiple files', function () {
    ...
    return Promise.all(
      ...
      .then(function () {
        //Commit the change with execute bit set
        return exec("git commit -m 'test'", { cwd: test.repository.workdir() });
      })
      .then(function () {
