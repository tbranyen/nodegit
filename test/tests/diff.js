...
describe('Diff', function () {
  ...
  beforeEach(function () {
    ...
    return Repository.open(reposPath)
      ...
      .then(function () {
        return Diff.treeToWorkdirWithIndex(test.repository, test.masterCommitTree, {
      ...
      .catch(function (e) {
        return fse.remove(diffFilepath).then(function () {
  ...
  it('can diff the workdir with index', function () {
    return this.workdirDiff.patches().then(function (patches) {
  ...
  it('can resolve individual line changes from the patch hunks', function () {
    return this.workdirDiff
      ...
      .then(function (patches) {
        ...
        return Promise.all(hunkPromises).then(function () {
          return result;
        });
      })
      .then(function (hunks) {
  ...
  it('can diff index to workdir', function () {
    return this.indexToWorkdirDiff.patches().then(function (patches) {
      assert.equal(patches.length, 3);
    });
  });
  ...
  it('can merge two commit diffs', function () {
    ...
    return this.repository
      ...
      .then(function (_secondDiff) {
        ...
        return Promise.all([getLinesFromDiff(firstDiff), getLinesFromDiff(secondDiff)]);
      })
  ...
  describe('merge between commit diff and workdir and index diff', function () {
    beforeEach(function () {
      var test = this;
      return fse
        .writeFile(path.join(test.repository.workdir(), 'newFile.txt'), 'some line\n')
        .then(function () {
        ...
        .then(function (headCommit) {
          return Promise.all([headCommit.getTree(), test.repository.index()]);
        })
    ...
    it('can merge a diff from a commit into a diff from a work dir and index', function () {
      ...
      return Promise.all([
        ...
        .then(function (linesOfMergedDiff) {
          var allDiffLines = _.flatten([linesOfWorkDirWithIndexDiff, linesOfCommitDiff]);
          _.forEach(allDiffLines, function (diffLine) {
            assert.ok(_.includes(linesOfMergedDiff, diffLine));
          });
        });
