...
describe('Filter', function () {
  ...
  afterEach(function () {
    return Registry.unregister(filterName).catch(function (error) {
  ...
  describe('Register', function () {
    ...
    after(function (done) {
      Registry.unregister(secondFilter).then(function () {
        done();
      });
    });

    it('can register a filter', function () {
      return Registry.register(filterName, mockFilter, 0).then(function (result) {
  ...
  describe('Unregister', function () {
    ...
    it('can unregister the filter', function () {
      return Registry.unregister(filterName).then(function (result) {
        assert.strictEqual(result, NodeGit.Error.CODE.OK);
      });
    });
  ...
  describe('Apply', function () {
    before(function () {
      var test = this;
      return fse.readFile(readmePath, 'utf8').then(function (content) {
    ...
    it('should not apply when check returns GIT_PASSTHROUGH', function () {
      ...
      return Registry.register(
        ...
        .then(function (result) {
          assert.strictEqual(result, NodeGit.Error.CODE.OK);
          return fse.writeFile(packageJsonPath, 'Changing content to trigger checkout', {
            encoding: 'utf-8',
          });
        })
        .then(function () {
    ...
    it('does not apply when GIT_PASSTHROUGH is returned', function () {
      ...
      return Registry.register(
        ...
        .then(function () {
          var postInitializeReadmeContents = fse.readFileSync(readmePath, 'utf-8');
      ...
      it('applies the massive filter data on checkout', function () {
        ...
        return Registry.register(
          ...
          .then(function () {
            ...
            var readLength = fse.readSync(fd, readBuf, 0, largeBufferSize, 0);
            fse.closeSync(fd);
      ...
      return Registry.register(
        filterName,
        {
          ...
          check: function (src, _attr) {
            return src.path() === 'README.md' ? 0 : NodeGit.Error.CODE.PASSTHROUGH;
          },
        },
        0
      )
        ...
        .then(function () {
          return commitFile(test.repository, 'README.md', 'testing commit contents', 'test commit');
        })
        .then(function (_oid) {
        ...
        .then(function (commit) {
          ...
          assert.strictEqual(postInitializeReadmeContents, 'testing commit contents');
      ...
      return Registry.register(
        filterName,
        {
          apply: function (to, from, source) {
            return source.repo().then(function () {
              return NodeGit.Error.CODE.PASSTHROUGH;
            });
          },
