...
describe('Branch', function () {
  ...
  beforeEach(function () {
    ...
    return Repository.open(reposPath)
      ...
      .then(function (branch) {
        test.branch = branch;
        return test.repository.createBranch(branchName2, test.masterCommit, true);
      });
  });
  ...
  it('can get the name of a branch', function () {
    ...
    return NodeGit.Branch.name(branch).then(function (branchNameToTest) {
  ...
  it('can get the remote name of a branch', function () {
    ...
    return NodeGit.Branch.remoteName(repo, fullUpstreamName).then(function (remoteNameToTest) {
  ...
  it('can rename a branch', function () {
    ...
    return Branch.move(branch, branchName2, 0).then(
      function (_branch) {
        return Promise.reject(new Error('should not be able to rename the branch'));
      },
      function (_error) {
  ...
  it('can create branch from annotated commit', function () {
    ...
    return AnnotatedCommit.fromRevspec(repo, nonHeadCommit)
      .then(function (theAnnotatedCommit) {
        ...
        return Branch.createFromAnnotated(repo, branchName, annotatedCommit, 0);
      })
      .then(
        function (_branch) {
          return Promise.reject(new Error('should not be able to create the branch'));
        },
        function (_error) {
          return Promise.resolve()
            .then(function () {
              // force the branch creation
              return Branch.createFromAnnotated(repo, branchName, annotatedCommit, 1);
            })
            .then(function (branch) {
              assert.equal(branch.name(), fullBranchName);
            });
