...
describe('Remote', function () {
  ...
  it('can delete a remote', function () {
    ...
    return Remote.create(repository, 'origin3', url)
      ...
      .then(function () {
        return (
          Remote.lookup(repository, 'origin3')
  ...
  it('can download from a remote', function () {
    ...
    return repo.getRemote('origin').then(function (remote) {
      ...
      return remote
        .connect(NodeGit.Enums.DIRECTION.FETCH, remoteCallbacks)
        .then(function () {
  ...
  it('can monitor transfer progress while pushing', function () {
    ...
    return Remote.create(repo, 'bare', bareReposPath).then(function (remote) {
      var fetchOpts = {
        callbacks: {
      ...
      return remote.push(refs, fetchOpts).then(function (_res) {
        assert.ok(wasCalled);
      });
    });
  });

  it('can monitor transfer progress while pushing with throttling', function () {
    var repo = this.repository;
    var wasCalled = false;
  ...
  if (!isNode8) {
    it('will reject if credentials promise rejects', function () {
      ...
      return Remote.lookup(repo, 'origin')
        ...
        .then(
          function () {
            return Promise.reject(new Error('should not be able to push to the repository'));
          },
          function (err) {
                ...
                credentials: function (url, userName) {
                  if (firstPass) {
                    ...
                    if (url.indexOf('https') === -1) {
                      ...
                      return NodeGit.Credential.userpassPlaintextNew(userName, '');
                    }
                  } else {
  ...
  it('is kept alive by refspec', function () {
    ...
    repo.getRemote('origin').then(function (_remote) {
      remote = _remote;
    ...
    return promise.then(function () {
      ...
      assert.equal(startSelfFreeingCount, Remote.getSelfFreeingInstanceCount());
    });
  });
  ...
  it('can retrieve the list of references advertised by a remote', function () {
    ...
    return this.repository
      .getRemote('origin')
      .then(function (remote) {
        return Promise.all([remote, remote.connect(NodeGit.Enums.DIRECTION.FETCH)]);
      })
      ...
      .then(function (results) {
        ...
        fp.flow([
          ...
          fp.forEach(function (remoteHeadName) {
            assert(
              fp.isEqual(expectedRemoteHeads[remoteHeadName], remoteHeadsBySha[remoteHeadName]),
