...
describe('Status', function () {
  ...
  it('gets a status on changing file directory', function () {
    ...
    return fse
      ...
      .catch(function (e) {
        return fse.writeFile(filePath, oldContent).then(function () {
  ...
  it('gets status on non-existent file results in error', function () {
    ...
    var repo = this.repository;
    var filePath = path.join(repo.workdir(), fileName);
    return exec('git clean -xdf', { cwd: reposPath }).then(function () {
      assert.equal(false, fse.existsSync(filePath));
      return Status.file(repo, filePath).then(
        function () {
        ...
        function (err) {
          ...
          assert.equal("attempt to get status of nonexistent file '" + filePath + "'", err.message);
        }
      );
    });
  });
