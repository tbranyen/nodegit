...
describe('Reset', function () {
  ...
  function resetFrom(repo, commit, resetType, annotated) {
    var promise = null;
    if (annotated) {
      promise = AnnotatedCommit.lookup(repo, commit.id()).then(function (annotatedCommit) {
  ...
  it('can perform a soft reset', function () {
    ...
    return resetFrom(test.repo, test.previousCommit, Reset.TYPE.SOFT, false).then(function (blob) {
      var currentCommitContents = test.currentCommitBlob.toString();
      var previousCommitContents = test.previousCommitBlob.toString();
      var resetContents = blob.toString();
  ...
  it('can perform an annotated soft reset', function () {
    ...
    return resetFrom(test.repo, test.previousCommit, Reset.TYPE.SOFT, true).then(function (blob) {
      var currentCommitContents = test.currentCommitBlob.toString();
      var previousCommitContents = test.previousCommitBlob.toString();
      var resetContents = blob.toString();
  ...
  it('reset fails if parameter is not a Commit object', function () {
    ...
    try {
      ...
      assert.fail('Should not be able to pass a Promise (Commit) into the function');
    } catch (err) {
      // ok
      assert.equal("Repository and target commit's repository does not match", err.message);
    }
  });
