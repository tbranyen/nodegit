...
describe('Signature', function () {
  ...
  it('can get a default signature when no user name is set', function () {
    ...
    var cleanUp = () => {
      return exec('git config --global user.name "' + savedUserName + '"').then(() => {
  ...
  it('duplicates time', function () {
    ...
    var startNonSelfFreeingCount = Time.getNonSelfFreeingConstructedCount();
  ...
  it('toString provides the correct timestamp when requested', function () {
    ...
    assert.equal(signature.toString(true), 'Shaggy Rogers <shaggy@mystery.com> 987654321 +0130');
  });
});
